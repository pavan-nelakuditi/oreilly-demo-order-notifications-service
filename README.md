# OReilly Order Notifications Service

This repo contains the standalone order notifications service for the OReilly
workshop model.
...
- `SPECHUB_WRITE_TOKEN`: token with write access to `pavan-nelakuditi/Spechub` so the workflow can publish `dist/openapi.bundled.yaml` to `order-notifications/openapi.yaml`
